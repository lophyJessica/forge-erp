...
import type { SourceBalance } from '../types/finance';
...
export default function ReceiptForm() {
  ...
  const customers = financeApi.getCustomers();
  const [customerCode, setCustomerCode] = useState(params.get('customerCode') || customers[0]?.code || '');
  ...
  const openSources = useMemo(() => financeApi.getOpenReceivableSources(customerCode), [customerCode]);
  ...
  const submit = () => {
    try {
      setError('');
      const record = financeApi.createReceipt({ customerCode, sourceNo, amount, receiptDate, remark });
      alert(`收款成功：${record.id}`);
      navigate('/finance/receipts');
    } catch (err: any) {
      setError(err?.message || '收款失败');
    }
  };
  ...
  return (
    <div className="space-y-4 pb-16">
      <div className="flex items-center gap-3 bg-white p-4 rounded-lg shadow-sm border border-slate-100">
        <button onClick={() => navigate('/finance/receipts')} className="p-1 hover:bg-slate-100 rounded">
          <ArrowLeft size={18} className="text-slate-600" />
        </button>
        <div>
          <h1 className="text-lg font-bold text-slate-800">新建收款单</h1>
          <p className="text-xs text-slate-500 mt-1">选择未核销 SOO，收款金额不能超过未核销余额。</p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100 space-y-5 max-w-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="space-y-1">
            <span className="text-xs font-bold text-slate-500">关联客户</span>
            <select value={customerCode} onChange={event => setCustomerCode(event.target.value)} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-xs">
              {customers.map(customer => <option key={customer.code} value={customer.code}>{customer.code} {customer.name}</option>)}
            </select>
          </label>
          <label className="space-y-1">
            <span className="text-xs font-bold text-slate-500">收款日期</span>
            <Input type="date" value={receiptDate} onChange={event => setReceiptDate(event.target.value)} className="h-10 text-xs" />
          </label>
        </div>

        <label className="space-y-1 block">
          <span className="text-xs font-bold text-slate-500">选择未核销 SOO</span>
          <select
            value={sourceNo}
            onChange={event => {
        ...
        {selectedSource && (
          <div className="grid grid-cols-3 gap-3">
            ...
            <InfoCard label="已收金额" value={money(selectedSource.settledAmount)} tone="text-emerald-600" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="space-y-1">
            <span className="text-xs font-bold text-slate-500">收款金额</span>
        ...
        <div className="flex justify-end gap-2 border-t border-slate-100 pt-4">
          <Button variant="outline" onClick={() => navigate('/finance/receipts')}>取消</Button>
